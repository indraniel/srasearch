...
func (ar AccessionRecord) String() string {
	...
	sratype := fmt.Sprintf("Type         : %s\n", ar.Type)
	submissionId := fmt.Sprintf("SubmissionID : %s\n", ar.Type)
	...
	return submissionId + alias + status + sratype + issues + updated +
		published + received + visibility + study + bioproject + sample +
		biosample + experiment + md5
}
